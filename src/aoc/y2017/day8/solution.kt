...
private val re = "(\\S+) (\\S+) (\\S+) if (\\S+) (\\S+) (\\S+)".toRegex()
private fun registers(input: List<String>, part2: Boolean = false): Any {
    val map = mutableMapOf<String, Int>()
    var max = 0
    input.map {
        re.findAll(it).map { a -> a.groupValues.drop(1) }
            .first()
            .let { b -> Command(b[0], if (b[1] == "inc") b[2].toInt() else -b[2].toInt(), b[3], b[4], b[5].toInt()) }
    }
        .forEach { c ->
